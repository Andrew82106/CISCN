...
from tsnet.model_core import Two_Stream_Net
from config import coordinates_cat, iou_threshs, window_nums_sum, ratios, N_list, input_size, model_name
import numpy as np
...
class MainNet(nn.Module):
    def __init__(self, proposalN, num_classes, channels):
        ...
        self.pretrained_model = Two_Stream_Net()
        self.rawcls_net = nn.Linear(channels, num_classes)
        self.APPM = APPM()

    def forward(self, x, status='test', DEVICE='cuda:0'):
        batch_size, _, _, _ = x.shape

        local_fm, local_embeddings, srm_image, srmatt11, srmatt12, srmatt21, srmatt22 = self.pretrained_model(x)  # [N, 2048]
        ...
        if status == "train" or status == "print":
            # window_imgs cls
            window_imgs = torch.zeros([batch_size, self.proposalN, 3, 160, 160]).to(DEVICE)
            for i in range(batch_size):
                for j in range(self.proposalN):
                    ...
                    window_imgs[i:i + 1, j] = F.interpolate(x[i:i + 1, :, x0:(x1 + 1), y0:(y1 + 1)],
                                                            ...
                                                            mode='bilinear',
                                                            align_corners=True)
            window_imgs = window_imgs.reshape(batch_size * self.proposalN, 3, 160, 160)
            win_image = window_imgs.detach()
            _, window_embeddings, win_srm, _, _, _, _ = self.pretrained_model(window_imgs.detach())  # [N*4, 2048]
        ...
        if status == "train":
            return proposalN_windows_scores, proposalN_windows_logits, proposalN_indices, \
                window_scores, local_logits, srm_image, window_embeddings, local_embeddings
        elif status == "print":
            return proposalN_windows_scores, proposalN_windows_logits, proposalN_indices, \
                window_scores, local_logits, srm_image, win_image, win_srm
        else:
            return proposalN_windows_scores, proposalN_windows_logits, proposalN_indices, \
                   window_scores, local_logits, srm_image

if __name__=="__main__":
    model = MainNet(proposalN=6, num_classes=2, channels=2048)
    input = torch.randn(12, 3, 320, 320)
    model(input)