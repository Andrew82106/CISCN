...
def cam(checkpoint_save_path, image_path, save_path):
    ...
    parser.add_argument("--savefig", type=str, default=save_path, help="Path to save figure")
    ...
    for idx, extractor in zip(range(1, len(cam_extractors) + 1), cam_extractors):
        ...
        plt.margins(0, 0)
        fig.savefig(args.savefig + "/activation_map.png", format='png', transparent=True, dpi=300, pad_inches=0)
        # if num_cols > 1:
        #     for _axes in axes:
        #         if args.rows > 1:
        #             for ax in _axes:
        ...
        plt.imshow(result)
        fig.savefig(args.savefig + "/heatmap.png", format='png', transparent=True, dpi=300, pad_inches=0)
        # ax = axes[idx // num_cols][idx % num_cols] if args.rows > 1 else axes[idx] if num_cols > 1 else axes
        # ax.imshow(result)
        # ax.set_title(extractor.__class__.__name__, size=12)
        # ax.set_title("Ours", size=12)
    ...
    return [args.savefig + "/activation_map.png", args.savefig + "/heatmap.png"]
